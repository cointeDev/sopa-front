...
import { Agenda } from '../agenda/agenda.component'; // <-- 1. IMPORTE O COMPONENTE AGENDA AQUI

// --- DEFINIÇÕES GLOBAIS ---
export interface Operacional {
    id: string; nome: string; cor: string; perfil: string; foto?: string; 
}
...
export interface Card {
    ...
    statusVisual?: 'finalizado' | 'alocado' | 'pendente'; etapasAtivas?: { [key: string]: boolean }; 
}
    ...
    drop(event: CdkDragDrop<Card[]>) {
        if (typeof event.item.data !== 'string') {
            if (event.previousContainer === event.container) {
                ...
                if (novaLista === 'concluido' || novaLista === 'publicado') { card.statusVisual = 'finalizado'; } 
                else { card.statusVisual = card.responsavel ? 'alocado' : 'pendente'; }
            }
        }
    }
